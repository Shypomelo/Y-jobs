...
import { OutboundEvent, EventItem } from "@/app/providers";
...
export default function Tab2GeneralInventory({ inventoryItems, outboundEvents, inboundEvents, isEditMode, onUpdateStock, onAddItem, onDeleteItem }: Props) {
    ...
    // Helper to calculate usage from outbound events
    const getUsedCount = (modelName: string) => {
        return outboundEvents
            .filter(e => e.status === "Verified" && !e.archived) // Only verified and current period
            .reduce((sum, event) => {
                // Check if any item in the event matches the model
                const matchedItems = event.items.filter((i: EventItem) => i.itemName === modelName);
                return sum + matchedItems.length;
            }, 0);
    };

    // Helper to calculate inbound from inbound events
    const getInCount = (modelName: string) => {
        return inboundEvents
            .filter(e => e.status === "Verified" && !e.archived && !isSeSource(e.source)) // General sources only
            .reduce((sum, event) => {
                const matchedItems = event.items.filter((i: InboundItem) => i.itemName === modelName);
                return sum + matchedItems.reduce((s, i) => s + i.quantity, 0);
            }, 0);
    };
                        ...
                        <tbody className="divide-y divide-stone-100 dark:divide-stone-800">
                            {inventoryItems.length === 0 ? (
                                ...
                                inventoryItems.map((item) => {
                                    const used = getUsedCount(item.model);
                                    const inCount = getInCount(item.model);
                                    const left = item.stock + inCount - used;
                                    return (
            ...
            {isAddModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="w-full max-w-sm bg-white dark:bg-stone-900 rounded-xl shadow-2xl p-6 animate-in fade-in zoom-in duration-200">
                        <h3 className="text-lg font-bold text-stone-800 dark:text-stone-100 mb-4">新增一般物料/耗材品項</h3>
